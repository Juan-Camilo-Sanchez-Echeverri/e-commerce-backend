export const COUPON_CHARACTERS_INVALID =
  'Code contains characters not allowed (0, O, o, L, l, I, i)';

export const COUPON_CODE_LENGTH =
  'The code exceeds the maximum allowable length of 6';

export const COUPON_EQUAL = 'Coupon code already exists';

export const COUPON_EXPIRED = 'Coupon has expired';

export const COUPON_FORMAT_INVALID = 'The code must be alphanumeric';

export const COUPON_LABEL_EXIST = 'Coupon label already exists';

export const COUPON_NOT_AVAILABLE = 'Coupon is not available';

export const COUPON_NOT_FOUND = 'Coupon not found';

export const DISCOUNT_IS_REQUIRED =
  'discountAmount or discountPercentage is required';

export const COUPON_RULE_CHANGE_NOT_ALLOWED =
  'Cannot change the rule of an existing coupon. Create a new one instead';

export const COUPON_ONE_RULE_PERMITTED = 'Only one rule is permitted';
